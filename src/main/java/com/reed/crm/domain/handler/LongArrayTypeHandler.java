package com.reed.crm.domain.handler;
...
/**
 * Long类型处理器 MyBatis使用类型处理器（type handler）将数据从数据库特定的数据类型转换为应用程序中的数据类型，
...
public class LongArrayTypeHandler extends BaseTypeHandler<Long[]> {
	...
	public void setNonNullParameter(PreparedStatement ps, int i,
		...
		Array arr = ps.getConnection().createArrayOf("int", parameter);
		ps.setArray(i, arr);
	}
